...
- ``kubeProxyReplacement=probe``: This option is only intended for a hybrid setup,
  ...
  the kernel does not support the socket-LB, then the ClusterIP translation
...
- ``kubeProxyReplacement=partial``: Similarly to ``probe``, this option is
  ...
  ``socketLB.enabled``, ``nodePort.enabled``, ``externalIPs.enabled``
...
the socket-LB operate (a source IP is not available there, as the endpoint
