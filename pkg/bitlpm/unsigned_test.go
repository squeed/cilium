...
// TestUnsignedRangeLookup tests looking up keys with
...
func TestUnsignedRangeLookup(t *testing.T) {
	ranges := [][]uint16Range{
		uint16Range65535,
		uint16Range0_65535,
		uint16Range1_65534,
	...
	for _, r := range ranges {
		...
		t.Run(name, func(t *testing.T) {
			...
			for _, pr := range rangeLookupMap {
				gotEntry := tu.RangeLookup(pr.prefix(), pr.start)
...
// TestUnsignedSearch tests searching for all keys
// that match a searched-for key and prefix.
func TestUnsignedSearch(t *testing.T) {
	...
	for i := uint(0); i < 16; i++ {
		...
		t.Run(entry, func(t *testing.T) {
			...
			tu.Search(i, 0, func(prefix uint, key uint16, v string) bool {
				gotRes = append(gotRes, v)
				return true
			})
			if !reflect.DeepEqual(expectedRes, gotRes) {
				t.Fatalf("Searching range %s, expected to get %v, but got: %v", entry, expectedRes, gotRes)
			}
		})
	}
}
...
func BenchmarkTrieRangeLookup(b *testing.B) {
	tri := NewUintTrie[uint32, *struct{}]()
	emptyS := &struct{}{}
	count := uint(0)
	// mimic adding 2 octets worth of addresses
	...
	b.ReportAllocs()
	b.ResetTimer()
	if tri.RangeLookup(24, 0xffff_0000) == nil {
		b.Fatal("expected valid lookup, but got nil")
	}
	for i := uint32(0); i < 255; i++ {
		upperOct := i << 8
		st := tri.RangeLookup(24, 0xffff_0000|upperOct)
		if st == nil {
			b.Fatal("expected valid lookup, but got nil")
		}
		for t := uint32(0); t < 255; t++ {
			st := tri.RangeLookup(32, 0xffff_0000|upperOct|t)
			if st == nil {
				b.Fatal("expected valid lookup, but got nil")
			}
		}
...
func FuzzUint8(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, sequence []byte) {
		...
		for seenK, seenV := range seen {
			val := tree.RangeLookup(uint(seenV.plen), seenV.k)
