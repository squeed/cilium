...
func (tu *trieUint[K, T]) RangeLookup(prefix uint, k K) T {
	return tu.t.RangeLookup(prefix, tu.getKey(k))
}

func (tu *trieUint[K, T]) Search(prefix uint, k K, f ForEachFunc[K, T]) {
	tu.t.Search(prefix, tu.getKey(k), func(prefix uint, k Key[K], v T) bool {
