...
//
// A prefix-length (hereafter "prefix"), in a prefix-key pair or in a RangeLookup, represents the
...
type Trie[K, T any] interface {
	...
	// key.
	Lookup(key K) T
	// RangeLookup lookups the key that matches to at least
	// the prefix argument, but will return the longest match.
	//
	// Note: If the prefix argument exceeds the Trie's maximum
	// prefix, it will be set to the Trie's maximum prefix.
	RangeLookup(prefix uint, key K) T
	// Search iterates over every prefix-key pair that contains
	// the argument prefix-key pair. If the ForEachFunc argument
	// returns false the search will stop.
	//
	// Note: If the prefix argument exceeds the Trie's maximum
	// prefix, it will be set to the Trie's maximum prefix.
	Search(prefix uint, key K, fn ForEachFunc[K, T])
	// Upsert updates or inserts the trie with a a prefix, key, and value.
	//
	// Note: If the prefix argument exceeds the Trie's maximum
	// prefix, it will be set to the Trie's maximum prefix.
...
// all Trie types. A return of "false" stops iteration.
...
func (t *trie[K, T]) Lookup(k Key[K]) T {
	...
	var ret T
	t.Search(t.maxPrefix, k, func(prefix uint, k Key[K], v T) bool {
		ret = v
		return true
	})
	return ret
...
// RangeLookup returns the longest or most "specific" key that matches the key
// and prefix being looked up.
func (t *trie[K, T]) RangeLookup(prefixLen uint, k Key[K]) T {
	// default return value
	var ret T
	t.Search(prefixLen, k, func(prefix uint, k Key[K], v T) bool {
		ret = v
		return true
	})
	return ret
...
// Search iterates over every prefix-key pair that contains the
// argument prefix-key pair in order from shortest to longest prefix
// match. If the ForEachFunc argument returns false the search will
// stop.
//
// Search starts traversal at the root key (or "node") in the trie.
...
//
// Note: Search sets any prefixLen argument that exceeds the maximum
// prefix allowed by the trie to the maximum prefix allowed by the
// trie.
func (t *trie[K, T]) Search(prefixLen uint, k Key[K], f ForEachFunc[Key[K], T]) {
	if k == nil {
		return
	}
	...
	for currentNode != nil {
		...
		if matchLen == t.maxPrefix {
			if !currentNode.intermediate {
				f(currentNode.prefixLen, currentNode.key, currentNode.value)
			}
			return
		}
...
// `Search` comments for details on how this works). If the match
...
//     parent. If there is not parent then the node key is now the
//     root key (or "node").
//  2. The node key matches the upsert-node to the exact
//     prefix. Then the upsert key should replace the node key.
//  3. The node key matches the upsert key to the upate prefix,
//     but node prefix is greater than the upsert prefix. The
//     node key should become a child of the upsert key.
//  4. The node key does not match with the upsert key to either
//     the node prefix or the upsert prefix. An intermediate node
//     needs to be inserted that replaces the current position of the
//     node key, but give it a prefix of the match between the
//     upsert key and node key. The node key and upsert key
//     become siblings.
//
...
func (t *trie[K, T]) Upsert(prefixLen uint, k Key[K], value T) {
	...
	//    the current-node with an upsertd prefix of the match
